...
class SquareMap(wx.Panel):
    ...
    def OnKeyUp(self, event):
        ...
        try:
            ...
            log.info('Unable to find hot-map record for node %s',
                     self.selectedNode)
        else:
    ...
    def SetSelected(self, node, point=None, propagate=True):
        ...
        self.UpdateDrawing()
        if node:
            wx.PostEvent(self, SquareSelectionEvent(
                node=node, point=point, map=self))
    ...
    def SetHighlight(self, node, point=None, propagate=True):
        ...
        # TODO: restrict refresh to the squares for previous node and new
        # node...
        self.UpdateDrawing()
        if node and propagate:
            wx.PostEvent(self, SquareHighlightEvent(
                node=node, point=point, map=self))
    ...
    def TextForegroundForNode(self, node, depth=0):
        """Determine the text foreground colour to use to display the label of
           the given node"""
        if node == self.selectedNode:
            fg_colour = wx.SystemSettings_GetColour(
    ...
    def DrawBox(self, dc, node, x, y, w, h, hot_map, depth=0):
        ...
        dx, dy, dw, dh = x + self.margin, y + self.margin, w - \
            (self.margin * 2), h - (self.margin * 2)
...
#        self.DrawIconAndLabel(dc, node, x, y, w, h, depth)
        children_hot_map = []
        hot_map.append(
            (wx.Rect(int(x), int(y), int(w), int(h)), node, children_hot_map))
        x += self.padding
        y += self.padding
        w -= self.padding * 2
        h -= self.padding * 2
        ...
        if w > self.padding * 2 and h > self.padding * 2:
            ...
            if children:
                ...
                self.LayoutChildren(dc, children, node, x,
                                    y, w, h, children_hot_map, depth + 1)
            else:
    ...
    def LayoutChildren(self, dc, children, parent, x, y, w, h, hot_map, depth=0, node_sum=None):
        ...
        """
        if node_sum is None:
            nodes = [(self.adapter.value(node, parent), node)
        ...
        if total:
            if self.square_style and len(nodes) > 5:
                ...
                if head and tail:
                    ...
                    head_coord, tail_coord = split_box(
                        head_sum / float(total), x, y, w, h)
                    if head_coord:
                        self.LayoutChildren(
                            dc, head, parent, head_coord[0], head_coord[
                                1], head_coord[2], head_coord[3],
                            hot_map, depth,
                            node_sum=head_sum,
                        )
                    if tail_coord and coord_bigger_than_padding(
                        ...
                        self.LayoutChildren(
                            dc, tail, parent, tail_coord[0], tail_coord[
                                1], tail_coord[2], tail_coord[3],
                            hot_map, depth,
                            node_sum=tail_sum,
                        )
                    return
            ...
            if head_coord:
                self.DrawBox(
                    dc, firstNode, head_coord[0], head_coord[
                        1], head_coord[2], head_coord[3],
                    hot_map, depth
                )
            else:
            ...
            if len(nodes) > 1 and tail_coord and coord_bigger_than_padding(
                    tail_coord, self.padding + self.margin):
                self.LayoutChildren(
                    dc, nodes[:-1], parent,
